...
begin
  ...
  if CurUninstallStep = usPostUninstall then begin
    ...
    if (version.Major > 6) or ((version.Major = 6) and (version.Minor >= 1)) then begin
      ...
      ShellExec('runas', ExpandConstant('{app}\update-daemon.exe'), '--delete', '', SW_HIDE, ewWaitUntilTerminated, ErrorCode);
    end;
  end;
end;
...
begin
  if CurStep = ssPostInstall then begin
    ...
    if (version.Major > 6) or ((version.Major = 6) and (version.Minor >= 1)) then begin
      ...
      ShellExec('runas', ExpandConstant('{app}\update-daemon.exe'), '--install "Service for update documents editor."', '', SW_HIDE, ewWaitUntilTerminated, ErrorCode);
    end;
